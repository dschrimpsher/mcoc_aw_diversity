...
class BG(object):
    ...
    def countDuplicates(self, printout=False):
        ...
        for i in self.chosen:
            exists = list(
                filter(lambda existingCharacter: existingCharacter["character"] == i["character"], self.chosen))
            if len(exists) > 1:
                if printout:
    ...
    def sortByPower(self):
        for p in self.players:
            ...
            for c in p.champs:
                ...
                if count >= 20:
                    break
            print(averagePower, count)
            averagePower = averagePower / count
            p.averagePower = averagePower
        self.players = sorted(self.players, key=lambda player: player.averagePower, reverse=False)




    def optimize(self, duplicates=False):
        #1. find the weakest player and set.
        #2. find the next weakest player etc...
        self.sortByPower()
        self.buildByWeakest(False)

    def buildByWeakest(self, duplicates=False):
        while (len(self.chosen) < len(self.players) * 5):
            for p in self.players:
                for c in p.champs:
                    if (p.added < 5):
                        ...
                        if (c.attack == True):
                            ...
                            if len(exists) == 0:
                                ...
                                if c.pi > exists[0]['pi']+500 or c.defense:
